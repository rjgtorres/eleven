...
(defmethod add-card ((card card) pos (seq seq) (player player))
  (let* ((seq-order (if (intersection '(2 3) (mapcar #'face (content seq)))
	 ...
	 (addingjoker (iscardp :joker card))
         (seqwithoutjokers (remove-if-not #'(lambda (x) (iscardp :joker x)) (content seq))))
    (if addingjoker
        (progn
          (assert (< (length seqwithoutjokers) 2))
        ...
        (assert (equal (face card) (first (remove-duplicates (mapcar #'suite (content seq)))))))
    (when (numberp pos)
