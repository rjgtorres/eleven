...
(defgeneric add-card (card pos goal player))
...
(defmethod add-card ((card card) pos (trio trio) (player player))
  (declaim (ignore player))
...
(defmethod add-card ((card card) pos (seq seq) (player player))
  (let* ((seq-order (if (intersection '(2 3) (mapcar #'face (content seq)))
		       (butlast sequence-order)
		       (rest sequence-order)))
	 (card-order (position (face card) seq-order))
	 (isjoker (iscardp :joker card)))
   (when (numberp pos)
    (assert (not isjoker))
    ...
    (when (iscardp :ace card)
      ...
      (assert (zerop (length (remove-if-not #'(lambda (x) (iscardp :ace x)) (content seq))))))
    (when isjoker
      (assert (< (length (remove-if-not #'(lambda (x) (iscardp :joker x)) (content seq))) 2))
      (assert (not (or (when (> pos 0)
			  (iscardp :joker (nth (1- pos) (content seq))))
		       (when (< (1+ pos) (length (content seq)))
			  (iscardp :joker (nth (1+ pos) (content seq))))
		       (when (equal pos :begin)
			 (iscardp :joker (first (content seq))))
		       (when (equal pos :end)
			 (iscardp :joker (first (last (content seq)))))))))
  ...
  (if (iscardp :joker (nth pos (content seq)))
      (progn (push ()))
      (push card (nth pos (content trio)))))
